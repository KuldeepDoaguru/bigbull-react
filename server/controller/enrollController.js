...
module.exports = {
  createuser: async (req, res) => {
    try {
      ...
      const userExist = await enrollModel.findOne({ email });
      if (userExist) {
      ...
      const newUser = await enrollModel.create({
        name,
        email,
        mobileNo,
        pwd,
      ...
      if (newUser) {
        res.status(201).json({
          ...
          name: newUser.name,
          email: newUser.email,
          isAdmin: newUser.isAdmin, // Assuming you have an 'isAdmin' property
          token: generateToken(newUser._id), // Generate a token for the user
        });
      } else {
  ...
  updateuser: async function (req, res) {
    ...
    try {
      ...
      if (err.toString().includes("E11000 duplicate key error")) {
        ...
        err.toString().includes("Error: ENOENT: no such file or directory")
      ) {
        req.body.user_image = req.file.path;
        user.set({
          ...req.body,
      ...
      if (!useremail) {
        ...
        if (isMatch) {
          if (useremail.active === true) {
            ...
            const usermailWithToken = Object.assign({
            ...
            res
              .status(403)
              .send("your account is restricted please contact support");
          }
        } else {
          if (useremail.active === true) {
              ...
              res
                ...
                .send(
                  "Password is incorrect try again" +
                    `   failed attempt: ${useremail["login_attempt"]}`
                );
            }
          } else {
            res
  ...
  refreshtoken: async function (req, res) {
    ...
    jwt.verify(
      refreshToken,
      process.env.SECRET_REFERESH_KEY,
      function (err, decode) {
        if (err) {
    ...
    try {
      ...
      if (isMatch) {
        if (newPassword === confirmPassword) {
          ...
          res
            .status(400)
            .send("New password & confirm password does not match ✖");
        }
      } else {
  ...
  buyCourse: async function (req, res) {
    ...
    try {
      ...
      if (!user) {
        ...
        user.purchased_course = user.purchased_course.concat({
          course_id: course_id,
        });
        await user.save();
...
const ref = async function () {
  const randomNumber = Math.random()
    ...
    .substring(2, 10 + 2)
    .toUpperCase();
  const rf = await enrollModel.findOne(
    { referralCode: randomNumber },
    { referralCode: 1, _id: 0 }
  );
